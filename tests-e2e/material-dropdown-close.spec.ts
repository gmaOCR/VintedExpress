...
test.describe('Material dropdown close (e2e fixture)', () => {
  test('ferme le dropdown material via la logique de fermeture conservatrice', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const c = document.querySelector('[data-testid="material-multi-list-dropdown-content"]');
      if (c && c.classList.contains('hidden')) c.classList.remove('hidden');
    });
    await expect(
      page.locator('[data-testid="material-multi-list-dropdown-content"]'),
    ).toBeVisible();
    await page.evaluate(() => {
    ...
    const content = page.locator('[data-testid="material-multi-list-dropdown-content"]');
